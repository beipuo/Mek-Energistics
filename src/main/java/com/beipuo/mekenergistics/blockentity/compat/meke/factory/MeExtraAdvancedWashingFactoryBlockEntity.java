...
public class MeExtraAdvancedWashingFactoryBlockEntity extends TileEntityExtraWashingFactory implements MeExtraFactoryBridge.Owner {
    ...
    @Override public boolean pushPattern(IPatternDetails patternDetails, KeyCounter[] inputHolder) { return getMainNode().isActive() && getAvailablePatterns().contains(patternDetails) && MeExtraFactoryBridge.pushFluidChemical(this, patternDetails, inputHolder, this.fluidTank, this.inputChemicalTanks); }
    ...
    @Override protected boolean onUpdateServer() { boolean sendUpdatePacket = MeExtraFactoryBridge.drainOutputs(this); sendUpdatePacket |= super.onUpdateServer(); return MeExtraFactoryBridge.updateServer(this, sendUpdatePacket, this.outputChemicalTanks); }
